...
import { Loader2, Search, X, Calendar as CalendarIcon, Filter, XCircle, Save, AlertTriangle } from 'lucide-react';
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              ...
              <input 
                ...
                className="w-full bg-neutral-800 border border-neutral-700 rounded-lg p-2.5 text-white focus:ring-2 focus:ring-blue-500 outline-none scheme-dark"
              />
            </div>
          </div>
...
export default function DragDropCalendarBoard() {
  ...
  const { data: ordersData, isLoading } = useQuery({
    ...
    // Importante: Refetch on window focus garante que se outro user mudar, este atualiza
    refetchOnWindowFocus: true, 
    staleTime: 1000 * 60 // 1 minuto de cache antes de considerar velho
  });

  // 2. MUTATION (Gravação de Dados) - A forma correta de persistir
  const updateOrderMutation = useMutation({
    ...
    onSuccess: () => {
      // FORÇA O REFRESH DA LISTA APÓS GRAVAR COM SUCESSO
  ...
  const { calendar, ordersLeftPanel } = useMemo(() => {
    ...
    if (ordersData?.items) {
      ordersData.items.forEach((order: any) => {
        ...
        // REGRA: Se tem data válida, vai para o calendário. Se não, vai para a esquerda.
  ...
  const handleDrop = (e: React.DragEvent, day: number) => {
    ...
    const scheduledDate = new Date(year, month, day, 12, 0, 0); // Meio dia para evitar fuso horário
    
    // Se arrastar "Nova" para o calendário, por defeito muda para "Agendada"?
    // O pedido diz "Alterar estado no popup", mas ao arrastar convém ter um default lógico.
    // Vamos manter o estado atual, a menos que o utilizador mude no popup depois.
    
    updateOrderMutation.mutate({
      id: draggedOrder.id,
      payload: { 
        dataPrevistaDeCarga: scheduledDate.toISOString(),
        // Opcional: Se for estado 1, muda para 3 automaticamente? 
        // Se não quiser automático, remova a linha abaixo.
  ...
  const handleRemoveDate = (order: any, e: React.MouseEvent) => {
    e.stopPropagation();
    // REGRA: Apenas "Nova" (1) pode sair do calendário
    if (Number(order.estadoId) !== 1) {
      alert("Encomendas 'A Definir' ou 'Agendadas' não podem ser removidas do planeamento, apenas mudadas de dia.");
      return;
    }

    if(confirm("Remover do calendário e voltar para Pendentes?")) {
      updateOrderMutation.mutate({
        id: order.id,
  ...
  const getDaysInMonth = (date: Date) => {
    ...
    // Ajuste para começar à Segunda (1) em vez de Domingo (0)
    let startDay = firstDayObj.getDay(); 
    // Em JS 0 = Domingo. Queremos que a grelha mostre Seg-Sex.
    // O loop abaixo vai filtrar Sáb/Dom, por isso o startDay precisa de ser alinhado.
      ...
      <div className="flex-1 bg-neutral-900 rounded-xl border border-neutral-800 flex flex-col shadow-xl overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-4 bg-neutral-950/50">
          <div className="grid grid-cols-5 gap-3 h-full content-start">
            ...
            {Array.from({ length: 42 }).map((_, i) => { // 6 semanas * 7 dias = 42 iterações base
              // Lógica complexa para mapear dias corridos para grelha de 5 dias
              // Simplificação: Vamos iterar os dias do mês e ver se é dia útil
              return null; 
            })}
            ...
            {(() => {
              ...
              for (let d = 1; d <= totalDays; d++) {
                ...
                const weekDay = dateCheck.getDay(); // 0=Dom, 6=Sab
                      ...
                      <div className="space-y-2 overflow-y-auto flex-1 custom-scrollbar-mini pr-1">
                        {dayOrders.map((o: any) => (
                          ...
                          >
                            ...
                            {/* BOTÃO REMOVER (X) - APENAS SE ESTADO FOR NOVA (1) */}
                            {Number(o.estadoId) === 1 && (
                              <button 
